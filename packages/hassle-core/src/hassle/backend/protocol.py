"""The `Backend` Protocol (F2) — DESIGN §8, §4; frozen at the start of M5.
...
so that M5's `FakeBackend` (in-memory, this milestone) and M6's `DirectBackend`
(real REST + WebSocket transport) can each be built and tested independently
against the same contract — neither needs to inherit from anything.
...
Deliberately NOT here (M6/DirectBackend-only concerns, unrelated to sync):
...
class Backend(Protocol):
    """What the sync engine needs from a Home Assistant transport (F2)."""
