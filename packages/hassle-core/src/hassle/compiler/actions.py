...
def service(
    ...
    **fields: Any,
    ...
    ``data_template=`` (residue-coverage round 2, docs/ha-api-notes.md §20):
...
def delay(*, alias: str | None = None, enabled: bool | None = None, **duration: Any) -> None:
    ...
    per-step name/toggle, keyword-only so they never collide with a duration
    unit passed via ``**duration``.
    """
