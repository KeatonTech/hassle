"""``IR -> DSL Python`` decompiler (DESIGN §7.3).
...
- :func:`analyze_coverage` — the DSL-coverage metric: walks the decompiled
  source per object and counts ``raw_*`` node usage.
...
object it cannot express as a typed decorator body) ``raw_automation`` — per
DESIGN §5.8, no data is ever dropped, only labeled as a coverage gap.
"""
