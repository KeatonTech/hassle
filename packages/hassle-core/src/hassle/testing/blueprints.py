"""Blueprint expansion for the simulator: `use_blueprint` -> a concrete config.
...
#: Bundle-relative directory holding blueprint sources, as path segments.
#: Mirrors HA's ``config/blueprints/automation/``; a ``use_blueprint`` path is
#: appended verbatim underneath it.
BLUEPRINT_SUBDIR: tuple[str, ...] = ("blueprints", "automation")
...
def blueprint_file(bundle_root: Path, use_blueprint_path: str) -> Path:
    ...
    """
    return bundle_root.joinpath(*BLUEPRINT_SUBDIR, use_blueprint_path)


def blueprint_display_path(use_blueprint_path: str) -> str:
    """The bundle-relative path shown in error messages (snapshot-stable —
    never an absolute checkout path)."""
    return "/".join((*BLUEPRINT_SUBDIR, use_blueprint_path))
