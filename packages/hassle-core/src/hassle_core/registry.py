"""Entity indexing form — the runtime object behind ``hassle.registry.entities``.

DESIGN §5.2/§5.3: ``from hassle.registry import entities as e`` gives ``e.<domain>``
a domain accessor exposing every object_id both as an attribute
(``e.sensor.hall_motion``) and by indexing (``e.sensor["hall_motion"]``), always
resolving to the same entity reference (an :class:`EntityRef`, the same
``str``-subclass type helper declarations already return — accepted anywhere the
DSL expects an entity id).
...
resolve to the identical :class:`EntityRef`.
"""
