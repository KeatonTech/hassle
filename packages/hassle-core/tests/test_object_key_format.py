...
def test_object_kinds_cover_automation_script_and_helpers() -> None:
    ...
    assert "blueprint" in OBJECT_KINDS
    # automation + script + dashboard + blueprint + 9 storage helper domains +
    # 4 template domains + 12 group domains == 29 kinds
    assert len(OBJECT_KINDS) == 4 + len(HELPER_DOMAINS) + len(TEMPLATE_DOMAINS) + len(GROUP_DOMAINS)
