"""`CATEGORY` module global drives the DISPLAY NAME a push-create's category
write-back uses when it has to create a brand-new HA category (the
`humanize_slug` fallback stays the behavior when no `CATEGORY` is supplied at
all).
...
handled entirely at the call site: ignore the global, warn -- write-back
additionally ignores the global with a warning, never guesses which side is
right). This module tests the
...
def test_attempt_category_writeback_uses_override_with_punctuation_verbatim() -> None:
    """Regression: the override's slug is NEVER expected to equal the source
    path's slug -- that
...
def test_apply_plan_with_no_category_overrides_keeps_m11_behavior() -> None:
    """No CATEGORY global at all -> byte-identical to the `humanize_slug`
    fallback (no `category_overrides` argument at all, the default)."""
    backend = FakeBackend()
...
def test_attempt_category_writeback_override_only_used_for_create_not_reuse() -> None:
    ...
    pre-existing matching category is still reused untouched (the "never
    guess which side is right" reuse rule is unaffected by CATEGORY)."""
    backend = FakeBackend()
