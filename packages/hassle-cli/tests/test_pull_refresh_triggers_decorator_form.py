"""`hassle pull`'s REFRESH splice emits the `triggers=` decorator form on
disk -- the real end-to-end path ("spliced objects now emit decorator-form
too").
...
unrelated bug means a REFRESH's on-disk result isn't actually a
byte-preserving LibCST splice in this test's harness (no `# hassle: updated
from UI on` marker survives, and a second object sharing the file gets
dropped) -- this test only asserts on the DECOMPILED CONTENT (decorator-form
triggers, no `when()`, no `# --- triggers ---`), not on splice-preservation
mechanics (already covered, with hand-written fixtures immune to the
pre-existing bug, by `packages/hassle-core/tests/test_splice.py`).
"""
...
def test_pull_refresh_splices_decorator_form_triggers(
    ...
    # The result must itself still compile (compile(decompile(x)) == x /
    # round-trip sanity).
