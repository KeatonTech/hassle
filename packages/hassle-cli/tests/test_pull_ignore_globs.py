...
def _append_ignore(bundle: Path, *globs: str) -> None:
    ...
    # git_repo's fixture tree is committed clean; re-commit so appending the
    # ignore glob doesn't itself trip the `pull` clean-tree check this test
    # isn't about.
    if (bundle / ".git").is_dir():
        subprocess.run(["git", "add", "-A"], cwd=bundle, check=True, capture_output=True)
        subprocess.run(
            ["git", "commit", "-q", "-m", "add ignore glob"],
            cwd=bundle,
            check=True,
            capture_output=True,
        )
...
def test_push_never_deletes_an_ignored_object_absent_locally(
    ...
    assert "input_boolean:material_you_primary_color" in backend.list_remote("input_boolean") or (
        "material_you_primary_color" in backend.list_remote("input_boolean")
    )
    # No write touched the ignored object (pushing the pre-existing hallway
    # automation is the only expected write here, if any).
    assert backend.writes_since_reset() == writes_before
