"""Can a BATCH-level self-check (compile just the objects going into one
destination file, before writing it) catch a decompiler coordination bug
earlier/more precisely than the CLI-level whole-bundle backstop
(`test_pull_post_write_compile_backstop.py`)?
...
**Value-comparison widening, then a fix:** the self-check was widened to
compare recompiled VALUES against the original stored config, not just
...
buggy: it wasn't context-free, and false-positived on a real bundle
...
def test_adopt_batch_self_check_catches_value_mismatch_that_still_compiles(monkeypatch) -> None:
    """The "does it compile" self-check alone would NOT have caught the real
    `for_each` template-string bug, because
...
def test_adopt_batch_self_check_does_not_false_positive_on_batch_context_text_diff(
    ...
    """Regression: the self-check's comparison must be context-free -- it
    must not care that the SAME object's decompiled TEXT differs between the
    real batch context (name-collision `_2` dedup suffix, or a same-batch
    script-call rewrite) and a solo re-decompile of the original config
    (fresh naming, no resolver). Reproduced here with a hand-rolled fake
    `decompile_bundle`: the batch output uses a `_2`-suffixed function name
    (as a real batch would, when a second object's alias collides) for
    `automation:a1`, but a solo `decompile_object` of the SAME stored config
    always names it fresh (`a1`, no collision partner) -- a pure
    Python-identifier difference that can never appear in `to_ha()` (the
    function name is not part of the HA value at all). The OLD
    text-comparison self-check would have treated this as a value mismatch
    and blocked the whole batch (six objects refused, only one of which --
    if any -- was a real problem); the value-comparison self-check must not.
    """
    from hassle.sync import pull_apply as pull_apply_mod
...
def test_real_bundle_shape_class_pulls_clean_via_cli(
    ...
    shape class -- an automation with a same-batch shared-script call,
    adopted together with five sibling helpers (two `input_datetime`, two
    `input_number`, one `input_text`) in one pull -- must pull clean. This
    is the exact class of object the earlier bug blocked (six objects
    refused; `hassle pull` left the bundle empty). Fixed via `values_match`'s
    context-free canonical-JSON comparison."""
    import subprocess
