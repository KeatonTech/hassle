...
is always caller-supplied (R8, docs/backend.md) -- the CLI is the only layer
allowed to call a clock, so `now_iso()` lives here, not in hassle-core.
"""
...
def now_iso() -> str:
    """The only wall-clock call in the CLI layer (R8: core logic never calls
