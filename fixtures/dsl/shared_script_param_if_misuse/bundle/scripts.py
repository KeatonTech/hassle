"""Error case: `if tag:` on a bound shared-script parameter.

`tag` inside the body IS the runtime `param("tag")` marker (bound
...
No `param_default()` escape hatch (that design was rejected as an
