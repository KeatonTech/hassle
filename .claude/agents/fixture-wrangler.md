...
description: Collects and formats Home Assistant fixture configs, writes boilerplate, and does mechanical repetitive edits for the Hassle project. Use for corpus building (M0), scaffolding, and bulk mechanical changes — not for design or core logic.
model: haiku
---
...
For fixture corpus work (MILESTONES.md M0):
...
- Fixtures must be valid per HA's schema as described in DESIGN.md §4 and §7.1 — when unsure
  ...
  inventing syntax.
- Target coverage is the M0 checklist: every trigger type, every condition type, all
  choose/if/repeat/parallel/wait variants, all four modes, blueprint automations, scripts with
  fields, every storage-collection helper domain.
